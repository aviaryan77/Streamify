  ...
  return (
    <div>
      ...
      <VStack minH={['auto', 'auto', '64vh', '64vh']}>
        {SIDEBAR_DATA?.map((nav) => {
          return (
            <CN_Link
              key={nav.title}
              href={nav.href}
              className='w-8/12 px-5 py-2 my-2 rounded-full bg-gradient-to-r from-fuchsia-400 to-blue-400 justify-center'
            >
              {nav.title}
            </CN_Link>
          );
        ...
        <button
          onClick={changeTheme}
          className='w-8/12 px-5 py-2 my-3 rounded-full bg-gradient-to-r from-fuchsia-400 to-blue-400'
        >
          Toggle Theme
        </button>
        <button
